...
export default function Budget({ segments }: Props) {
  const [budget, setBudget] = useState(142500000);
  ...
  const budgetPct = ((budget - 9500000) / (475000000 - 9500000)) * 100;
      ...
      <div style={{ background: '#fff', borderRadius: 20, padding: 28, boxShadow: '0 2px 12px rgba(0,0,0,0.06)', border: '1px solid #f0f0f0', marginBottom: 20 }}>
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          <div style={{ position: 'relative' }}>
            ...
            <input type="range" min={9500000} max={475000000} step={4750000} value={budget}
              onChange={e => setBudget(parseInt(e.target.value))}
              style={{ position: 'absolute', inset: 0, opacity: 0, cursor: 'pointer', width: '100%' }} />
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 6 }}>
            <span style={{ fontSize: 11, color: '#d1d5db' }}>₹95 L</span>
            <span style={{ fontSize: 11, color: '#d1d5db' }}>₹47.5 Cr</span>
          </div>
        </div>
