...
const KPICard = ({ label, value, sub, delta, accent, span }: { label: string; value: string | number; sub?: string; delta?: string; accent?: string; span?: number }) => (
  <div style={{
    background: '#fff',
    borderRadius: 20,
    padding: '20px 24px',
  ...
  return (
    <div style={{ padding: '28px 32px', background: '#f8f9fb', minHeight: '100vh', fontFamily: 'DM Sans, sans-serif' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 16, marginBottom: 24 }}>
        <KPICard label="Health Score" value={health.health_score} sub={`Grade ${health.health_grade}`} accent={gradeColor} span={2} />
        ...
        <KPICard label="Total Repair Cost" value={fmtCost(parseFloat(health.total_repair_cost_usd))} sub={`${fmtCost(parseFloat(health.urgent_repair_cost_usd))} urgent`} span={2} />
